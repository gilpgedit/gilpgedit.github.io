...
export const multi_element_tag = "multi-element"
const MULTI_ITEM_CLASS = "multi-eleent-iten"

/**
 * @template {HTMLElement} ElementType
 * @template ValueType
 * @typedef {Object} MultiElementOnly
 ...
 *                         LiteElementProperties<MultiElement> &
 *                               MultiElementOnly<ElementType, ValueType>
 ...
 *           LiteElementParameter<MultiElement<ElementType, ValueType>,
 *                  MultiElementProperties<ElementType, ValueType>>[]} contents
 */
export function Multi(...contents) {
 /**
  * @type {MultiElement<ElementType, ValueType>}
  */
 const element = new MultiElement()
 ...
 * @template ValueType
 */
export class MultiElement extends HTMLElement {
 ...
 static cast(element) {
  if (element instanceof MultiElement) {
   ...
   throw new Error(badElementTypeTemplate(multi_element_tag))
  }
 }
 ...
 set name(name) {
  ...
  const elements = this._list.querySelectorAll(MULTI_ITEM_CLASS)
 ...
 _add() {
  ...
  element.classList.add(MULTI_ITEM_CLASS)
...
customElements.define(multi_element_tag, MultiElement)